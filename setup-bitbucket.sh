...
drupal9ci_install() {
	...
	set -ex
	# Download and extract Bitbucket Pipelines configuration and sample tests.
	wget -O "$tmpdir/master.zip" https://github.com/lullabot/drupal9ci/archive/master.zip
	unzip "$tmpdir/master.zip" 'drupal9ci-master/dist/bitbucket/*' -d "$tmpdir"
	rsync -va --ignore-existing "$tmpdir/drupal9ci-master/dist/bitbucket/" .
